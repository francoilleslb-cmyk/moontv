...
// 🗑️ BORRAR TODAS LAS SERIES (endpoint específico - va ANTES de /:id)
router.delete('/delete-all', async (req, res) => {
  try {
    ...
    console.log(`🗑️ Eliminadas ${result.deletedCount} series`);
    ...
    console.error('❌ Error en DELETE /delete-all series:', err);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (!s) return res.status(404).json({ success: false, message: 'No encontrada' });
    res.json({ success: true, message: `"${s.title}" eliminada` });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
